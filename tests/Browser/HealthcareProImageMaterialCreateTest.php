...
class HealthcareProImageMaterialCreateTest extends DuskTestCase
{
    ...
    public function testBasicExample()
    {
        ...
        $this->browse(function (Browser $browser) use ($material_count, $storagePath) {
            $browser->clickLink('Materiais')
                    ...
                    ->clickLink('Nova Imagem')
                    ->assertPathIs('/caregivers/public/materials/create/image')
                    ->type('name', 'test')
                    ->type('description', 'test description')
                    ->attach('pathImage', $storagePath.'/images/Imagem-1.jpg')
                    ->press('Criar')
                    ->assertPathIs('/caregivers/public/materials');

            $material_count_new = count(Material::all());
            $new_material = Material::find($material_count_new);
            $new_material->type = 'Imagem';
            ...
            $browser->assertSeeIn('table tr:first-child td:first-child', $new_material->name)
                    ...
                    ->assertVisible('img[src=\'http://192.168.99.100/caregivers/public/materials/'.$new_material->id.'/showContent\']')
